...
**Compaction boundary.** `_find_keep_boundary` always lands on a `user` message — never split an `assistant(tool_calls)` + `tool` pair, otherwise the next OpenAI request 400s. It keeps as many complete recent user turns as fit under 60% of the history budget, capped at 6 and always keeping at least the latest complete user→assistant/tool turn. The rest becomes one `[历史摘要]` system message at the head of `agent.history`. Token budget is `role.llm.history_token_budget` falling back to `settings.llm.default_history_token_budget`.
