...
python scripts/smoke_prompt_cache.py             # stable notebook/handoff prefixes + cache-aware compaction
...
**Handoff notes are one-shot and append-only.** `agent.queue_context_note` puts a string into `pending_context_notes`; `Agent.handle` consumes the notes into a clearly marked `[系统上下文通知 — 非用户输入]` block prepended to the next user message. The combined user turn is persisted exactly once. Do NOT inject handoff notes as leading system messages: inserting anything before an existing role history invalidates its prefix cache. On turn rollback the consumed notes are restored to the pending queue.

**Notebook TOC is a stable prefix snapshot.** Every Agent snapshots `notebook.toc()` + a content/index revision when it is materialised. `_build_system_messages` reads only that snapshot, never the live file. Cross-role writes are detected at the target Agent's next `handle` and delivered through the same append-only context block as handoffs; the writer suppresses its own redundant notification because its tool result already contains the update. A successful compaction refreshes the full TOC snapshot because the history prefix is already changing at that boundary, but deliberately does NOT mark an unseen revision as seen: the next user turn still gets one explicit update notice, including same-key overwrites whose TOC text did not change. `Notebook.revision()` hashes the bounded notebook + index files rather than relying on day-resolution timestamps.
...
**Cache-aware compaction.** Agent records the exact last successful `CompletionRequest` and the history length represented by it. When compaction runs, the summarizer reuses the same messages/model/tools/settings, appends the assistant/tool messages produced after that request, then adds one `[系统维护任务：历史压缩]` system instruction at the tail. The old agent request is therefore an exact prefix and can hit upstream KV cache. Tool calls returned by this maintenance request are rejected and never executed. Agents with no prior request use the old sterile two-message fallback. After replacement, the old request snapshot is cleared; the next ordinary turn establishes the new compacted prefix.

**Compaction boundary.** `_find_keep_boundary` always lands on a `user` message — never split an `assistant(tool_calls)` + `tool` pair, otherwise the next OpenAI request 400s. It keeps as many complete recent user turns as fit under 60% of the history budget, capped at 6 and always keeping at least the latest complete user→assistant/tool turn. The rest becomes one `[历史摘要]` system message at the head of `agent.history`. Token budget is `role.llm.history_token_budget` falling back to `settings.llm.default_history_token_budget`. If an unchanged single-turn history remains over budget and has no safe boundary, the compactor emits one WARNING for that `(history length, tokens, budget)` state rather than repeating it after every other role's turn.
...
**Team profile injection.** `~/.chat_team/team.md` is read once by `load_settings` into `settings.team_profile` (stripped); when non-empty, `Agent._build_system_messages` splices it as a `[团队信息]` block alongside the role prompt and meta lines. Empty/missing file → no block, behaviour unchanged. Cache-aware compaction reuses the exact prior agent request (including its profile) and appends a maintenance instruction that says to summarize conversation facts only; the no-prior-request fallback retains the sterile standalone compactor prompt. Hot-reloadable: `chat-team --reload` (or `kill -HUP <pid>`) re-reads `team.md` into `settings.team_profile` in place; the next turn's system-prompt rebuild picks it up without a restart. See "Hot reload" below.

**Runaway-output guards.** Two cooperating protections against a model "looping" on its own output (collapsing into verbatim repetition until the upstream cap). (1) `llm/repetition.py` watches the *streaming* accumulation — content text AND each tool-call's argument JSON — and when the tail becomes N exact copies of one block (KMP prefix-function period detection, block-size agnostic, checked every 256 chars) the provider closes the stream and raises `_DegenerateOutputError`, which is in `_RETRYABLE_EXCEPTIONS`: the normal retry loop re-samples on the same key (a fresh sample almost always breaks the loop), then rotates keys. Exact-match only, so merely *similar* lines (tables, numbered lists) never trip it; `scripts/smoke_output_guards.py` pins the false-positive cases. (2) `llm.chat.max_tokens` (default **16384**, 0 = no cap) is the hard backstop, forwarded as the `max_tokens` kwarg on agent-turn requests; cache-aware compaction inherits the agent's value via the reused request snapshot. A response truncated by the cap logs WARNING `finish_reason=length`. Vision/OCR calls are deliberately uncapped (short outputs; the repetition detector still protects them).
...
- **Don't put standalone context system messages into `agent.history`.** The leading system content comes from `Agent._build_system_messages` (role prompt + stable notebook snapshot). Handoff/notebook updates must be merged into the next user turn through `queue_context_note`, preserving append-only cache semantics. The compactor's summary head is the sole persisted system-message exception; its tail maintenance instruction exists only in the transient compaction request.
